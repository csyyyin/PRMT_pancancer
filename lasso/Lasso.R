...
Tumor <- c("ACC","BLCA","BRCA","CESC","CHOL","COAD","DLBC","ESCA","GBM","HNSC","KICH","KIRC","KIRP","LAML","LGG","LIHC","LUAD","LUSC","MESO","OV","PAAD","PCPG","PRAD","READ","SARC","SKCM","STAD","TGCT","THCA","THYM","UCEC","UCS","UVM")
...
{
  all_sur_data <- read.csv('D:/PRMT/lasso/survival.csv',header=T,check.names=F)
  all_sur_data <- all_sur_data[grep(name_fold,all_sur_data$CODE),][,-c(1)]
  
  gene_exp <- read.csv('D:/PRMT/lasso/PRMT.csv')
