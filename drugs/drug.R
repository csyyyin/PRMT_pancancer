...
GDSC2_Expr = readRDS(file='D:/PRMT/yaowu/GDSC2_Expr (RMA Normalized and Log Transformed).rds')
GDSC2_Res = readRDS(file = "D:/PRMT/yaowu/GDSC2_Res.rds")
GDSC2_Res = exp(GDSC2_Res)
tumor_id <- "ACC"
exp = read.csv('D:/PRMT/yaowu/PRMT.csv',header = T,check.names=F)
...
for (tumor_id in tumor_list)
{
  ...
  temp_text <- read.csv('D:/PRMT/yaowu/calcPhenotype_Output/DrugPredictions.csv',header=T,row.names=1,check.names=F)
