...
for(classify in data_list)
{
  ...
  out_put_list <- c()
  file_name = paste0('D:/PRMT/check/',classify,'/cluster')
  ...
  my_data = read.csv(paste0('D:/PRMT/check/',classify,'.csv'),header=T,check.names=F)
  
  other_data = read.csv("D:/PRMT/check/cluster.csv",header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    pdf(paste("D:/PRMT/check/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 10)
  ...
  write.csv(p_value_csv,file=paste("D:/PRMT/check/p.csv",sep=''),quote=F)
  
  ######
  pdf(paste("D:/PRMT/check/fig1.pdf",sep=''),width = length(colnames(exp_file))*4,height = 8*length(out_put_list)/3.5)
