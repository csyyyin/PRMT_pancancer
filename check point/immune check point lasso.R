...
for(classify in data_list)
{
  ...
  out_put_list <- c()
  file_name = paste0('D:/PRMT/check/',classify,'/risk')
  ...
  other_data = read.csv("D:/PRMT/check/risk.csv",header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    pdf(paste("D:/PRMT/check/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
