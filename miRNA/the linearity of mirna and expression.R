...
{
  print(tumor_id)
  dat_input <- read.table(paste0("D:/PRMT/miRNA/rstudio-export/",tumor_id),sep='\t',check.names=F,header=T,row.names=1)
  ...
  data_all_tumor <- read.csv(paste("D:/PRMT/miRNA/PRMT.csv",sep=''),check.names = F)
  ...
  pdf(paste('D:/PRMT/miRNA/',tumor_id,".pdf",sep=''),width = 7,height = 10)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数据
    scale_color_gradient2(low = '#006600', high = '#1D2088') +  # 调整颜色渐变,
...
##拼图
pdf("fig1.pdf",width = length(colnames(dat_input))/40,height = length(colnames(dat_one_tumor))*length(out_put_list)/15)
