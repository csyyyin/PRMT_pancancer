...
for(classify in data_list)
{
  ...
  out_put_list <- c()
  file_name = paste0('D:/PRMT/immune_reg/',classify,'/cluster')
  
  if (!dir.exists(file_name)){
    dir.create(paste0('D:/PRMT/immune_reg/',classify))
  ...
  other_data = read.csv("D:/PRMT/immune_reg/cluster.csv",header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    pdf(paste("D:/PRMT/immune_reg/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 10)
    #填充
