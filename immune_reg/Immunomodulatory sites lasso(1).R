...
for(classify in data_list)
{
  ...
  out_put_list <- c()
  file_name = paste0('D:/PRMT/immune_reg/',classify,'/risk')
  ...
  my_data = read.csv(paste0('D:/PRMT/immune_reg/',classify,'.csv'),header=T,check.names=F)
  
  other_data = read.csv("D:/PRMT/immune_reg/risk.csv",header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    exp_file <- subset(exp_file,select=-c(CODE,SampleName,Group))
    ...
    pdf(paste("D:/PRMT/immune_reg/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
  ...
  write.csv(p_value_csv,file=paste("D:/PRMT/immune_reg/p.csv",sep=''),quote=F)
  
  ######
  pdf(paste("D:/PRMT/immune_reg/fig1.pdf",sep=''),width = length(colnames(exp_file))*4,height = 8*length(out_put_list)/3.5)
