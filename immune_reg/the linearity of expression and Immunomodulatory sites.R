...
for(classify in data_list)
{
  ...
  file_name = paste0('D:/PRMT/immune_reg/',classify,'/corr')
  ...
  for (tumor_id in tumor_list)
  {
    ...
    dat_input <- read.csv(paste0('D:/PRMT/immune_reg/',classify,'.csv'),header=T,check.names=F)
    ...
    data_all_tumor <- read.csv(paste("D:/PRMT/immune_reg/PRMT.csv",sep=''),check.names = F)
    ...
    pdf(paste("D:/PRMT/immune_reg/1/",tumor_id,".pdf",sep=''),width = 15,height = round(length(colnames(dat_input))/2.87,0))
    p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
      geom_point() +  # 使用点表示数据
      scale_color_gradient2(low = '#EA5514', high = '#006934') +  # 调整颜色渐变
  ...
  ##拼图
  pdf(paste("D:/PRMT/reg/1/","fig1.pdf",sep=''),width = 56,height = length(out_put_list)*round(length(colnames(dat_input))/2.87,0)/4)
