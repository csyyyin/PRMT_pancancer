...
other_data = read.csv("D:/PRMT/CIBER/risk.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/CIBER/1/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
...
######
pdf(paste("D:/PRMT/CIBER/1/fig1.pdf",sep=''),width = length(colnames(exp_file))*3.5,height = 8*length(out_put_list)/3.5)
