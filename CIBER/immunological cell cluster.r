...
my_data = read.csv('D:/PRMT/CIBER/CIBER.csv',header=T,check.names=F)

other_data = read.csv("D:/PRMT/CIBER/cluster.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/CIBER/1/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 10)
...
write.csv(p_value_csv,file=paste("D:/PRMT/CIBER/p.csv",sep=''),quote=F)

######
pdf(paste("D:/PRMT/CIBER/fig1.pdf",sep=''),width = length(colnames(exp_file))*4,height = 8*length(out_put_list)/3.5)
