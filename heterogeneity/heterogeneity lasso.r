...
out_put_list <- c()
file_name = 'D:/PRMT/yizhix/'
final_tumor_list = c()

my_data = read.csv('D:/PRMT/yizhix/yzx.csv',header=T,check.names=F)

other_data = read.csv("D:/PRMT/yizhix/risk.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/yizhix/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
...
######
pdf(paste("D:/PRMT/yizhix/fig1.pdf",sep=''),width = length(colnames(exp_file))*4+3,height = 8*length(out_put_list)/4)
