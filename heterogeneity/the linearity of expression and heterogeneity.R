...
for (tumor_id in tumor_list)
{
  ...
  dat_input <- read.csv("D:/PRMT/yizhix/yzx.csv",check.names = F,header=T)
  ...
  data_all_tumor <- read.csv(paste("D:/PRMT/yizhix/PRMT.csv",sep=''),check.names = F)
  ...
  pdf(paste('D:/PRMT/yizhix/',tumor_id,".pdf",sep=''),width = 15,height = 5)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数据
    scale_color_gradient2(low = '#009D91', high = '#CC33FF') +  # 调整颜色渐变,
