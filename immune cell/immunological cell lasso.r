...
other_data = read.csv("D:/PRMT/immune cell/risk.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/immune cell/1/",name,".pdf",sep=''),width=length(unique(dat[,1]))+3,height = 8)
...
######
pdf(paste("D:/PRMT/immune cell/1/fig1.pdf",sep=''),width = length(colnames(exp_file))*10,height =10*length(out_put_list)/3.5)
