...
for (tumor_id in tumor_list)
{
  ...
  dat_input <- read.csv("D:/PRMT/immune cell/gx.csv",check.names = F,header=T)
  ...
  data_all_tumor <- read.csv(paste("D:/PRMT/immune cell/ESTIMATE.csv",sep=''),check.names = F)
  ...
  pdf(paste('D:/PRMT/immune cell/corr/',tumor_id,".pdf",sep=''),width = 15,height = 5)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数据
    scale_color_gradient2(low = "#1D2088", high = "#C46F0F") +  # 调整颜色渐变
...
##拼图
pdf("D:/PRMT/immune cell/corr/fig1.pdf",width = 50,height = round(length(out_put_list)/5,0)*10)
