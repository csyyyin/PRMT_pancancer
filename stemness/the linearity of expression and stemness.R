...
for (tumor_id in tumor_list)
{
  ...
  dat_input <- read.csv("D:/PRMT/ganx/gx.csv",check.names = F,header=T)
  ...
  data_all_tumor <- read.csv(paste("D:/PRMT/ganx/PRMT.csv",sep=''),check.names = F)
  ...
  pdf(paste(tumor_id,".pdf",sep=''),width = 15,height = 5)
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +  # 使用点表示数据
    scale_color_gradient2(low = "#920783", high = "#FF6633") +  # 调整颜色渐变
...
##拼图
pdf("fig1.pdf",width = 50,height = round(length(out_put_list)/5,0)*10)
