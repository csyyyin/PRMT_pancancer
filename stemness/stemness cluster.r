...
my_data = read.csv('D:/PRMT/ganxing/gx.csv',header=T,check.names=F)

other_data = read.csv("D:/PRMT/ganxing/cluster.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/ganxing/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
...
write.csv(p_value_csv,file=paste("D:/PRMT/ganxing/p.csv",sep=''),quote=F)

######
pdf(paste("D:/PRMT/ganxing/fig1.pdf",sep=''),width = length(colnames(exp_file))*4+3,height = 8*length(out_put_list)/4)
