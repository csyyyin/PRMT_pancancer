...
out_put_list <- c()
file_name = 'D:/PRMT/fenqi'
final_tumor_list = c()

my_data = read.csv('D:/PRMT/fenqi/PRMT.csv',header=T,check.names=F)

other_data = read.csv("D:/PRMT/fenqi/M.csv",header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste("D:/PRMT/fenqi/1/",name,".pdf",sep=''),width=length(unique(dat[,1])),height = 8)
...
write.csv(p_value_csv,file=paste("D:/PRMT/fenqi/p.csv",sep=''),quote=F)

######
pdf(paste("D:/PRMT/fenqi/1/fig1.pdf",sep=''),width = length(colnames(exp_file))*3.5,height = 8*length(out_put_list)/4)
